...
export function Veiligheid() {
  ...
  const getCrimData = (card: typeof crimCijfersCard) => {
    const crim = card.overrideData
      ? { ...card.overrideData, _jaar: undefined }
      : gebiedData.criminaliteit;
    const jaar = card.overrideData?._jaar ?? gebiedData.dataJaar;
    return { crim, jaar };
  };

  // Default data (voor score + pie chart — altijd uit gebiedData)
  const bevolking = gebiedData.bevolking;
  const criminaliteit = gebiedData.criminaliteit;
  const dataJaar = gebiedData.dataJaar;
  ...
  const calculateVeiligheidsScore = (): { score: number; misdrijvenPer1000: number } | null => {
    ...
    return { score: Math.round(score * 10) / 10, misdrijvenPer1000: Math.round(misdrijvenPer1000) };
  };
  ...
  // Tab score met de actieve benchmarks (schakelt mee met de toggle)
  const tabScore = berekenVeiligheidScore(gebiedData, benchmarks);

  // Data per individuele card
  const cijfers = getCrimData(crimCijfersCard);
  const vermogen = getCrimData(vermogenCard);
  const geweld = getCrimData(geweldCard);
  const overlast = getCrimData(overlastCard);
  const verkeer = getCrimData(verkeerCard);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '32px', flexWrap: 'wrap' }}>
            ...
            {gebiedData.veiligheidsVergelijking && (
              <div style={{ display: 'flex', gap: '8px', flex: 1, minWidth: '200px', flexWrap: 'wrap' }}>
                <ScoreColumn label="Buurt" score={gebiedData.veiligheidsVergelijking.buurt?.score} naam={gebiedData.veiligheidsVergelijking.buurt?.naam} isActive={selectedGebied?.type === 'buurt'} />
                <ScoreColumn label="Wijk" score={gebiedData.veiligheidsVergelijking.wijk?.score} naam={gebiedData.veiligheidsVergelijking.wijk?.naam} isActive={selectedGebied?.type === 'wijk'} />
                <ScoreColumn label="Gemeente" score={gebiedData.veiligheidsVergelijking.gemeente?.score} naam={gebiedData.veiligheidsVergelijking.gemeente?.naam} isActive={selectedGebied?.type === 'gemeente'} />
                <ScoreColumn label="Nederland" score={gebiedData.veiligheidsVergelijking.nederland?.score} naam={gebiedData.veiligheidsVergelijking.nederland?.naam} isActive={false} />
              </div>
            )}
          </div>
        </div>
                ...
                <LineChart data={gebiedData.criminaliteitTrend.jaren} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(value, name) => {
                      const labels: Record<string, string> = { vermogen: 'Vermogensdelicten', geweld: 'Geweldsdelicten', vernieling: 'Vernielingen', verkeer: 'Verkeer' };
                      return [value, labels[String(name)] || name];
                    }}
                  />
                  <Legend wrapperStyle={{ fontSize: '11px' }} formatter={(value: string) => {
      ...
      <SelectableCard sectionId="veiligheid-cijfers" title="Criminaliteitscijfers" badge="data" year={cijfers.jaar}
        ...
        ) : crimCijfersCard.activeMode === 'trend' && crimCijfersCard.trendData ? (
          <CardTrendChart
            data={crimCijfersCard.trendData}
            lines={[
              { key: 'totaal', label: 'Totaal', color: '#1d1d1b' },
              { key: 'vermogen', label: 'Vermogen', color: '#eb6608' },
              { key: 'geweld', label: 'Geweld', color: '#e74c3c' },
              { key: 'vernieling', label: 'Vernieling', color: '#3498db' },
              { key: 'verkeer', label: 'Verkeer', color: '#f39c12' },
            ]}
          />
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '12px' }}>
            <StatBox label="Totaal" value={cijfers.crim.totaal} highlight />
            <StatBox label="Vermogensdelicten" value={cijfers.crim.vermogen} />
            <StatBox label="Geweldsdelicten" value={cijfers.crim.geweld} />
            <StatBox label="Vernielingen" value={cijfers.crim.vernieling} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '20px' }}>
        ...
        <SelectableCard sectionId="veiligheid-vermogen" title="Vermogensdelicten" badge="data" year={vermogen.jaar}
          ...
          {vermogenCard.isLoading ? (
            <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>Laden...</div>
          ) : vermogenCard.activeMode === 'trend' && vermogenCard.trendData ? (
            <CardTrendChart data={vermogenCard.trendData} lines={[{ key: 'vermogen', label: 'Vermogensdelicten', color: '#eb6608' }]} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <MeldingRow label="Inbraak woningen" value={vermogen.crim.inbraakWoningen} />
              <MeldingRow label="Inbraak schuur/garage" value={vermogen.crim.inbraakSchuur} />
              <MeldingRow label="Inbraak bedrijven" value={vermogen.crim.inbraakBedrijven} />
              <MeldingRow label="Diefstal auto's" value={vermogen.crim.dieftalAutos} />
              <MeldingRow label="Diefstal uit auto's" value={vermogen.crim.dieftalUitAutos} />
              <MeldingRow label="Fietsendiefstal" value={vermogen.crim.dieftalFietsen} />
              <MeldingRow label="Zakkenrollerij" value={vermogen.crim.zakkenrollerij} />
        ...
        <SelectableCard sectionId="veiligheid-geweld" title="Geweldsdelicten" badge="data" year={geweld.jaar}
          ...
          {geweldCard.isLoading ? (
            <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>Laden...</div>
          ) : geweldCard.activeMode === 'trend' && geweldCard.trendData ? (
            <CardTrendChart data={geweldCard.trendData} lines={[{ key: 'geweld', label: 'Geweldsdelicten', color: '#e74c3c' }]} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <MeldingRow label="Mishandeling" value={geweld.crim.mishandeling} />
              <MeldingRow label="Bedreiging" value={geweld.crim.bedreiging} />
              <MeldingRow label="Openlijk geweld" value={geweld.crim.openlijkGeweld} />
              <MeldingRow label="Straatroof" value={geweld.crim.straatroof} />
              <MeldingRow label="Overval" value={geweld.crim.overval} />
              <MeldingRow label="Zedenmisdrijf" value={geweld.crim.zedenmisdrijf} />
        ...
        <SelectableCard sectionId="veiligheid-overlast" title="Overlast & Overig" badge="data" year={overlast.jaar}
          ...
          {overlastCard.isLoading ? (
            <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>Laden...</div>
          ) : overlastCard.activeMode === 'trend' && overlastCard.trendData ? (
            <CardTrendChart data={overlastCard.trendData} lines={[{ key: 'vernieling', label: 'Vernieling', color: '#3498db' }]} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <MeldingRow label="Vernieling" value={overlast.crim.vernieling} />
              <MeldingRow label="Drugs/drankoverlast" value={overlast.crim.drugsOverlast} />
              <MeldingRow label="Burengerucht" value={overlast.crim.burengerucht} />
              <MeldingRow label="Huisvredebreuk" value={overlast.crim.huisvredebreuk} />
              <MeldingRow label="Fraude" value={overlast.crim.fraude} />
              <MeldingRow label="Brand/ontploffing" value={overlast.crim.brandOntploffing} />
              <MeldingRow label="Openbare orde" value={overlast.crim.aantastingOpenbareOrde} />
        ...
        <SelectableCard sectionId="veiligheid-verkeer" title="Verkeer" badge="data" year={verkeer.jaar}
          ...
          {verkeerCard.isLoading ? (
            <div style={{ padding: '20px', textAlign: 'center', color: '#6b7280', fontSize: '13px' }}>Laden...</div>
          ) : verkeerCard.activeMode === 'trend' && verkeerCard.trendData ? (
            <CardTrendChart data={verkeerCard.trendData} lines={[{ key: 'verkeer', label: 'Verkeer', color: '#f39c12' }]} />
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <MeldingRow label="Verkeersongevallen" value={verkeer.crim.verkeersOngevallen} />
...
function ScoreColumn({ label, score, naam, isActive }: { label: string; score?: number; naam?: string; isActive: boolean }) {
  const getScoreColor = (s: number) => s >= 7 ? '#22c55e' : s >= 5 ? '#f59e0b' : '#ef4444';
  return (
    <div style={{ flex: '1 1 70px', minWidth: '70px', textAlign: 'center', padding: '10px 6px', backgroundColor: isActive ? '#f5f1ee' : 'transparent', border: isActive ? '2px solid #eb6608' : '1px solid #e5e7eb' }}>
      <p style={{ fontSize: '11px', color: '#6b7280', marginBottom: '4px', fontWeight: 500 }}>{label}</p>
      {score !== undefined ? (
        <>
          <p style={{ fontSize: '22px', fontWeight: 700, color: getScoreColor(score), margin: 0, lineHeight: 1 }}>{score.toFixed(1)}</p>
...
function MeldingRow({ label, value, highlight = false }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 10px', backgroundColor: highlight ? '#eb6608' : '#f5f1ee', color: highlight ? 'white' : undefined, fontSize: '13px' }}>
...
function InfoIcon() {
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center' }} onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)}>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{ cursor: 'help' }}>
        <circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" />
      </svg>
      {showTooltip && (
        <div style={{ position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', marginTop: '8px', padding: '12px 16px', backgroundColor: '#1d1d1b', color: 'white', borderRadius: '4px', fontSize: '12px', lineHeight: '1.5', width: '320px', zIndex: 100, boxShadow: '0 4px 12px rgba(0,0,0,0.15)' }}>
          <p style={{ fontWeight: 600, marginBottom: '8px' }}>Berekening Veiligheidsscore</p>
          <p style={{ marginBottom: '8px' }}>De score is gebaseerd op <strong>gewogen</strong> criminaliteitscijfers per 1.000 inwoners.</p>
          <p style={{ marginBottom: '8px' }}><strong>High-impact delicten</strong> (geweld, woninginbraak) wegen <strong>2.5x zwaarder</strong> dan veelvoorkomende delicten (fietsendiefstal, vernieling).</p>
          <p style={{ marginBottom: '8px', fontSize: '11px', color: '#9ca3af' }}>Formule: Score = 10 - (gewogen misdrijven per 1000 / 12)</p>
          <p style={{ fontSize: '11px', color: '#9ca3af', marginBottom: '8px' }}>10 = zeer veilig | 7+ = veilig | 5 = gemiddeld | &lt;5 = onveilig</p>
          <a href="https://onderzoek.amsterdam.nl/dataset/cijfers-veiligheidsindex" target="_blank" rel="noopener noreferrer" style={{ fontSize: '11px', color: '#60a5fa', textDecoration: 'underline' }}>Meer over de methodologie</a>
          <div style={{ position: 'absolute', top: '-6px', left: '50%', transform: 'translateX(-50%)', width: 0, height: 0, borderLeft: '6px solid transparent', borderRight: '6px solid transparent', borderBottom: '6px solid #1d1d1b' }} />
        </div>
      )}
    </div>
  );
